...
module Components.Shadcn.Dialog where

import           Components.Icons      (dialogX)
import           Data.Functor.Identity (Identity)
import           Lucid                 (HtmlT, ToHtml (toHtml), class_, div_,
                                        id_, span_)
...
overlay = div_ [
  id_ "dialog-overlay",
  makeAttribute "data-state" "closed",
  class_ "fixed inset-0 z-50 bg-background/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0"
  ] ""
